"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const prisma_1 = require("../lib/prisma");
const auth_1 = require("../middleware/auth");
const router = (0, express_1.Router)();
// Simple in-memory cache for high-frequency active match polling
let activeMatchesCache = null;
let activeMatchesCacheTime = 0;
const CACHE_TTL_MS = 2500; // 2.5s cache prevents DB flooding from 3s client polling
// Get active matches (ongoing)
router.get('/active', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const now = Date.now();
        // Return cached response if within TTL
        if (activeMatchesCache && (now - activeMatchesCacheTime) < CACHE_TTL_MS) {
            return res.json(activeMatchesCache);
        }
        const matches = yield prisma_1.prisma.match.findMany({
            where: { status: 'IN_PROGRESS' },
            orderBy: { updatedAt: 'desc' },
            include: {
                participants: {
                    include: { player: { include: { user: { select: { name: true } } } } }
                }
            }
        });
        // Update cache
        activeMatchesCache = matches;
        activeMatchesCacheTime = now;
        res.json(matches);
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal error' });
    }
}));
// Get completed matches (history)
router.get('/history', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const matches = yield prisma_1.prisma.match.findMany({
            where: { status: 'COMPLETED' },
            orderBy: { updatedAt: 'desc' },
            include: {
                participants: {
                    include: { player: { include: { user: { select: { name: true } } } } }
                }
            }
        });
        res.json(matches);
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal error' });
    }
}));
// Get specific match
router.get('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const match = yield prisma_1.prisma.match.findUnique({
            where: { id: req.params.id },
            include: {
                participants: {
                    include: { player: { include: { user: { select: { name: true } } } } }
                },
                rallies: {
                    orderBy: { timestamp: 'asc' }
                }
            }
        });
        if (!match)
            return res.status(404).json({ error: 'Match not found' });
        res.json(match);
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal error' });
    }
}));
// Create new match
router.post('/', auth_1.authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { type, teamA, teamB, participants } = req.body;
    // participants should look like [{ playerId: '...', team: 'A' }, ...]
    try {
        const match = yield prisma_1.prisma.match.create({
            data: {
                type,
                teamA,
                teamB,
                participants: {
                    create: participants.map((p) => ({
                        playerId: p.playerId,
                        team: p.team
                    }))
                }
            },
            include: {
                participants: {
                    include: { player: { include: { user: { select: { name: true } } } } }
                }
            }
        });
        res.status(201).json(match);
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal error' });
    }
}));
// End match
router.post('/:id/end', auth_1.authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const { winnerTeam, scoreA, scoreB } = req.body;
    try {
        const match = yield prisma_1.prisma.match.update({
            where: { id: id },
            data: {
                status: 'COMPLETED',
                scoreA,
                scoreB
            },
            include: { participants: { include: { player: true } }, rallies: true }
        });
        const matchAny = match;
        // 1. Elo Calculation Logic
        const teamAPlayers = matchAny.participants.filter((p) => p.team === 'A');
        const teamBPlayers = matchAny.participants.filter((p) => p.team === 'B');
        // Average Elo for teams
        const eloA = teamAPlayers.reduce((sum, p) => sum + p.player.elo, 0) / (teamAPlayers.length || 1);
        const eloB = teamBPlayers.reduce((sum, p) => sum + p.player.elo, 0) / (teamBPlayers.length || 1);
        const K = 32;
        const expectedA = 1 / (1 + Math.pow(10, (eloB - eloA) / 400));
        const expectedB = 1 / (1 + Math.pow(10, (eloA - eloB) / 400));
        const actualA = winnerTeam === 'A' ? 1 : (winnerTeam === 'B' ? 0 : 0.5);
        const actualB = winnerTeam === 'B' ? 1 : (winnerTeam === 'A' ? 0 : 0.5);
        const eloChangeA = Math.round(K * (actualA - expectedA));
        const eloChangeB = Math.round(K * (actualB - expectedB));
        // 2. Update player stats
        for (const p of matchAny.participants) {
            const isWinner = p.team === winnerTeam;
            const eloChange = p.team === 'A' ? eloChangeA : eloChangeB;
            const smashes = matchAny.rallies.filter((r) => r.scoringPlayer === p.playerId && r.shotType === 'Smash').length;
            const drops = matchAny.rallies.filter((r) => r.scoringPlayer === p.playerId && r.shotType === 'Drop').length;
            const errorsCommitted = matchAny.rallies.filter((r) => r.opponentMistakePlayer === p.playerId).length;
            // Update Player Core Stats
            yield prisma_1.prisma.player.update({
                where: { id: p.playerId },
                data: {
                    elo: { increment: eloChange },
                    matchesPlayed: { increment: 1 },
                    wins: { increment: isWinner ? 1 : 0 },
                    losses: { increment: isWinner ? 0 : 1 },
                    smashPoints: { increment: smashes },
                    dropPoints: { increment: drops },
                    errorsCommitted: { increment: errorsCommitted }
                }
            });
            // Record specifically what +elO or -elO happened in THIS match
            yield prisma_1.prisma.matchParticipant.update({
                where: { id: p.id },
                data: { eloChange }
            });
        }
        res.json(match);
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal error' });
    }
}));
exports.default = router;
